...
std::cout << "out sums: ";
	...
	*/
	...
	unsigned int* h_block_out = new unsigned int[grid_sz];
	checkCudaErrors(cudaMemcpy(h_block_out, d_block_sums, sizeof(unsigned int) * grid_sz, cudaMemcpyDeviceToHost));
	std::cout << "out sums: ";
	for (int i = 0; i < grid_sz; ++i)
	{
		std::cout << h_block_sums_out[i] << std::endl;
