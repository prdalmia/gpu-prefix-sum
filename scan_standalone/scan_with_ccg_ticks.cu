	...
	const size_t numElems)
{
	...
	cudaMallocManaged(&time_b, sizeof(long long int ));
	// Sum scan data allocated to each block
	//gpu_sum_scan_blelloch<<<grid_sz, block_sz, sizeof(unsigned int) * max_elems_per_block >>>(d_out, d_in, d_block_sums, numElems);
	void *kernelArgs[] = {
        (void *)&d_out,  (void *)&d_in, (void *)&d_block_sums,  (void *)&d_block_sums_dummy, (void *)&d_block_sums_dummy_2,  (void *)&numElems, (void *)&shmem_sz, (void *)&max_elems_per_block, (void *)&time, (void *)&time_b 
	};
	int numBlocksPerSm;
	...
	cudaEvent_t start;
		...
		printf("time cuda only(ms) is %f and barrier time is %llu and total time is %llu \n", ms, *time_b, *time) ;
